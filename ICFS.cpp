...
#pragma omp parallel for private(i__)
	...
	*alpha = 0.; //#### HARI
	...
	for (i__ = 1; i__ <= i__1; ++i__)
	{
		...
		{
                        *alpha = std::max<double>(*alpha, -adiag[i__] * (wa2[i__] * wa2[i__]));
		}
	}
	if (*alpha > 0.)
	{
...
#pragma omp parallel for private(i__)
        ...
        /*        Attempt an incomplete factorization. */
                dicf_(n, nnz, &l[1], &ldiag[1], &lcol_ptr__[1], &lrow_ind__[1], p, &info, &iwa[1], &iwa[*n + 1], &iwa[(*n << 1) + 1], &wa1[1]);
		...
		if (info >= 0)
		{
			...
			/*           Otherwise undo the scaling of L and exit. */
                        if (*alpha == alphas && nb < -1) //####HARI
			{
                                alphas /= 4.; //####HARI
				*alpha = alphas;
                                std::cout<<"Shift at lower bound. Alpha reset to "<<*alpha<<std::endl;
				++nb;
			}
			else
			{
...
#pragma omp parallel for private(j)
		...
		else
		{
			...
			*alpha = std::max<double>(d__1, alphas);
                        std::cout<<"alpha adjusted to "<<*alpha<<std::endl;
		}
	}
	return 0;
} /* dicfs_ */
